...
| Local mistral runtime path (`mistral_local`)        | Explicit local `mistralrs-server` runtime route over `/v1/embeddings` | No                                         | Preferred local runtime path            |
...
- Embedding backend kinds (`http`/`openai_http`/`mistral_local`/`litellm_rs`): `packages/rust/crates/xiuxian-llm/src/embedding/backend.rs:1`
...
- Backend label decides transport family:
  ...
  - `mistral_local` (aliases `mistral`, `mistral_rs`, `mistral_server`) -> local `mistralrs-server` route
  - `litellm_rs` -> provider route (with Rust HTTP fallback chain)
- LLM backend labels now follow the same explicit split:
  - `mistral_local` stays first-class (not collapsed into generic `http`)
...
- Alias normalization (including `mistral*` -> `mistral_local`): `packages/rust/crates/xiuxian-llm/src/embedding/backend.rs:38`
...
- Gateway base URL resolution now distinguishes `mistral_local` and prefers `mistral.base_url` for that mode:
...
2. Treat backend policy as:
   ...
   - Local explicit runtime: `mistral_local` (managed by `xiuxian-llm::mistral`)
...
1. Add role-based benchmark gates:
   - `mistral_local`: cold start/warm start latency + sustained local throughput.
